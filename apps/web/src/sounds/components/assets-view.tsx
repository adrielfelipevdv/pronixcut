...
import { useLocalAudioLibraryStore } from "@/sounds/local-audio-library-store";
import type { AudioLibraryItem } from "@/sounds/local-audio-library-store";
...
const ACCEPTED_AUDIO_EXTENSIONS = ".mp3,.wav,.m4a,.aac,.ogg,.flac";
		...
		<div className="flex h-full flex-col">
			<Tabs defaultValue="sound-effects" className="flex h-full flex-col">
				<div className="px-3 pt-4 pb-0">
					<TabsList>
						<TabsTrigger value="sound-effects">Sounds & effects</TabsTrigger>
...
function LocalAudioLibraryView() {
	const { items, isLoading, isImporting, load, importFiles, remove, addToTimeline } =
		useLocalAudioLibraryStore();
	...
	const playItem = (item: AudioLibraryItem) => {
		...
		audioElement?.pause();
		const audio = new Audio(`/api/media-library/audio/file/${item.id}`);
		audio.addEventListener("ended", () => setPlayingId(null));
		audio.addEventListener("error", () => setPlayingId(null));
		audio.play().catch((error) => {
			console.error("Failed to play library audio:", error);
			setPlayingId(null);
		});
	...
	return (
		<div className="mt-1 flex h-full flex-col gap-4">
			...
			>
				...
				{isImporting ? "Importando..." : "Adicionar áudio"}
			</Button>
			...
			<div className="relative h-full overflow-hidden">
				<ScrollArea className="h-full flex-1">
					<div className="flex flex-col gap-4">
						{isLoading && (
							<div className="text-muted-foreground text-sm">Carregando biblioteca...</div>
						)}
						{!isLoading && items.length === 0 && (
							<div className="text-muted-foreground text-sm">
								Nenhum áudio na biblioteca ainda. Clique em "Adicionar áudio" para
								importar músicas e efeitos sonoros do seu computador.
							</div>
						)}
						{items.map((item) => (
							<LocalAudioItem
...
interface LocalAudioItemProps {
	...
	onAddToTimeline: (item: AudioLibraryItem) => void;
}

function LocalAudioItem({ item, isPlaying, onPlay, onRemove, onAddToTimeline }: LocalAudioItemProps) {
	return (
		<div className="group flex items-center gap-3 opacity-100 hover:opacity-75">
			...
			<div className="flex items-center gap-3 pr-2">
				...
				<Button
					variant="text"
					size="icon"
					className="text-muted-foreground w-auto !opacity-100 hover:text-red-500"
